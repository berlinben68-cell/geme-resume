...
def create_cover_letter_pdf(text, output_path, design_config=None):
    """
    Creates a professional PDF cover letter using FPDF.
    design_config: dict with keys 'color', 'font', 'template'
    """
    if not FPDF:
        print("Error: fpdf not installed.")
        return False

    if design_config is None:
        design_config = {'color': (0, 0, 0), 'font': 'Arial', 'template': 'Simple'}
    ...
    try:
        ...
        # Config
        font_name = design_config.get('font', 'Arial')
        # FPDF standard fonts: Courier, Helvetica, Times, Symbol, ZapfDingbats
        # Map common names to FPDF standard
        fpdf_font = 'Arial' # Default
        if 'Times' in font_name: fpdf_font = 'Times'
        elif 'Courier' in font_name: fpdf_font = 'Courier'
        
        accent_color = design_config.get('color', (0, 0, 0))
        ...
        # --- Header ---
        # Simple parsing of the text to find header info (Name, Contact) vs Body
        # Assuming the AI generates "Name\nContact\n\nDear..." structure or similar.
        # For now, we'll treat the first few lines as header if they are short.
        ...
        is_body = False
        for line in lines:
            if "Dear" in line or "To" in line and len(line) < 50:
                is_body = True
            
            if not is_body and len(line.strip()) > 0 and len(line) < 60:
...
def create_linkedin_report_pdf(data, output_path):
    """
    Creates a comprehensive LinkedIn Optimization Report PDF.
    """
    if not FPDF:
        print("Error: fpdf not installed.")
        return False
    ...
    try:
        ...
        SECONDARY_COLOR = (50, 50, 50) # Dark Grey
        ...
        pdf.set_text_color(*PRIMARY_COLOR)
        pdf.cell(0, 20, "LinkedIn Optimization Report", ln=True, align='C')
        
        pdf.set_font('Arial', '', 12)
        pdf.set_text_color(100, 100, 100)
        pdf.cell(0, 10, "Generated by DevCareer OS", ln=True, align='C')
        pdf.ln(10)
        
        # --- 1. SEO Strategy ---
        seo = data.get('seo_strategy_audit', {})
        if seo:
            add_section_header("1. SEO Strategy Audit")
            
            add_sub_header("Primary Keyword Cluster")
            for kw in seo.get('primary_keyword_cluster', []):
                pdf.cell(10) # Indent
                pdf.cell(0, 6, f"- {kw}", ln=True)
                
            add_sub_header("Keyword Gaps (Missing)")
            for gap in seo.get('keyword_gap_analysis', []):
                pdf.cell(10)
                pdf.cell(0, 6, f"- {gap}", ln=True)

        # --- 2. Text Optimization ---
        text_opt = data.get('text_optimization', {})
        if text_opt:
            add_section_header("2. Text Optimization")
            
            # Headlines
            add_sub_header("Headline Options")
            ...
            add_sub_header("About Section Strategy")
            pdf.set_font('Arial', 'I', 11)
            pdf.multi_cell(0, 6, f"Hook: {about.get('hook', '')}")
            pdf.ln(2)
            pdf.set_font('Arial', '', 11)
            pdf.multi_cell(0, 6, about.get('body', ''))
            
            # Experience
            exp = text_opt.get('experience_rewrites', {})
            add_sub_header(f"Experience Rewrite: {exp.get('role', 'Recent Role')}")
            for stmt in exp.get('impact_statements', []):
                pdf.multi_cell(0, 6, f"- {stmt}")
                pdf.ln(1)

        # --- 3. Visual Audit ---
        vis = data.get('visual_audit', {})
        if vis:
            add_section_header("3. Visual Brand Audit")
            
            photo = vis.get('profile_photo_check', {})
            add_sub_header(f"Profile Photo (Score: {photo.get('technical_score', '?')}/10)")
            pdf.multi_cell(0, 6, f"Impression: {photo.get('first_impression', '')}")
            pdf.multi_cell(0, 6, f"Fix: {photo.get('fix', '')}")
            
            banner = vis.get('banner_image_check', {})
            add_sub_header("Banner Image")
            pdf.multi_cell(0, 6, f"Relevance: {banner.get('relevance', '')}")
            pdf.multi_cell(0, 6, f"Recommendation: {banner.get('recommendation', '')}")

        # --- 4. URL & Settings ---
        url_audit = data.get('url_settings_audit', {})
        if url_audit:
            add_section_header("4. URL & Settings")
            opt = url_audit.get('url_optimization', {})
            pdf.multi_cell(0, 6, f"Status: {opt.get('status', '')}")
            pdf.multi_cell(0, 6, f"Fix: {opt.get('fix', '')}")

        pdf.output(output_path)
        return True
    except Exception as e:
        print(f"Error creating LinkedIn Report PDF: {e}")
        return False
